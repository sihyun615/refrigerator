...
function deleteColumn(boardTitle, columnIndex) {
  ...
  if (board && columnIndex !== undefined && columnIndex
...
// 카드 수정 모달 열기 함수
function showCardEditModal(boardTitle, columnIndex, cardIndex, cardTitle,
    cardContent, cardAssignee, cardDueDate) {
  // 기존에 열려 있는 모든 모달 닫기
  ...
  board.columns.forEach(function (column, columnIndex) {
    ...
    column.cards.forEach(function (card, cardIndex) {
      ...
      editCardButton.onclick = function () {
        showCardEditModal(board.title, columnIndex, cardIndex, card.title,
            card.content, card.assignee, card.dueDate);
      };
...
function addComment() {
  ...
  var board = boards.find(
      b => b.columns.some(c => c.cards.some(card => card.title === cardTitle)));
  if (board) {
    var card = board.columns.reduce(
        (found, col) => found ? found : col.cards.find(
            card => card.title === cardTitle), null);
    if (card) {
      // 댓글 객체 생성
      var comment = {
        text: commentInput,
  ...
  new Sortable(column.querySelector(".cards"), {
    ...
    onEnd: function (evt) {
      ...
      const newColumnIndex = Array.from(
          evt.to.closest('.columns').children).indexOf(
          evt.to.closest('.column'));
      const oldColumnIndex = Array.from(
          evt.from.closest('.columns').children).indexOf(
          evt.from.closest('.column'));
